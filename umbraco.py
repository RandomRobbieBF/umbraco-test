...
parser.add_argument("-f", "--file",required=False, help="File of URLS to check upload Against")
...
def test_page(newurl):
	...
	try:
		response = session.post(newurl, headers=headers,verify=False,data=burp0_data)
		if response.status_code == 200:
			if "soap:Envelope" in response.text:
